...
class UserGroupsController {
  ...
  static async getUserGroup(req, res, next) {
    try {
      const { id } = req.params;
      const resp = await User_Group.findOne({
        where: { id },
        include: [
          {
            model: Privilege,
  ...
  static async updateUserGroup(req, res, next) {
    try {
      ...
      const resp = await User_Group.update(
      ...
      if (!resp[1]) {
        ...
        res
          .status(200)
          .json({ message: `User Group with id ${id} has been updated` });
      }
    } catch (error) {
      next(error);
    }
  ...
  static async deleteUserGroup(req, res, next) {
    try {
      ...
      res
        .status(200)
        .json({ message: `User Group with id ${id} has been deleted` });
    } catch (error) {
      next(error);
    }
  }
